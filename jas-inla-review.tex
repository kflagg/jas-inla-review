...
\item Wrote up results in \ref{beiresults} and \ref{xsectresults}.

\item Removed figures that we probably won't discuss.
...
\caption{Plots of the event and covariate data for the real-world data
...
The residuals have small magnitude in the central region with no observed
events and low mean posterior intensity. This is not surprising because it is
not possible to see much variation in counts in that region. There are a
few large-magnitude positive residuals (about 4) suggetsing there is some
clustering not described by the model. Elsewhere, the residuals appear random
with no obvious spatial patterns. We repeated the residual analysis with
several grid cell sizes and saw the same charecteristics each time.
...
similar SD but larger range and less structure in predicted GP --- cannot
model smaller scale structure such as clustering
