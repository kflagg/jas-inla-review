...
include the integrated nested Laplace approximation (INLA) with 
...
% The paper also discuss VB and similar methods, identify limitations in
% applicability, point out bias in posterior variance that scales with the
% number of latent variables.
% Illustrates how to compute model comparison criteria and predictions.
% LGCP example using \texttt{bei} gridded to a 200 by 100 lattice,
% resulting in 40003 latent Gaussian variables.
% LGCP took 4 hours to fit, paper did not include MCMC results on full
% resolution lattice for comparison due to time.
% INLA can be further sped up with more recent advancements

% Kenny observation about terminology:
%   accuracy ~ convergeance
...
Section~\ref{into} provides an overview of spatial LGCP
...
% I think Taylor and Diggle (2014) should be worked into this paragraph, but
% don't know how. They did a simulation study comparing INLA to MCMC/MALA for
% fitting LGCP models after gridding and found INLA to be inferior.
...
Flagg et. al. used a Dirichlet Process to indentify hot spots with 
unexploded ordnances \cite{flagg2020}.
...
\caption{Plots of the event and covariate data for the Beilschmiedia Pendula Lauraceae data
analysis. The study region is 1000 m by 500 m.}
\label{bei}
\end{figure}

\subsection{Full Beilschmiedia Pendula Lauraceae Dataset}
\label{beianalysis}
...
\caption{Triangular mesh with pseudodata overlaid for the Beilschmiedia Pendula Lauraceae data
...
spatial GP from the Beilschmiedia Pendula Lauraceae data analysis.}
\label{beimean}
\end{figure}
...
using the posterior mean intensity from the Beilschmiedia Pendula Lauraceae data analysis.}
\label{beiintensity}
\end{figure}
...
\subfloat[Posterior mean of prediction surface.]
