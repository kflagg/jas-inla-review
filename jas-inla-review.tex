...
\caption{An illustration of the nodes and the weighting scheme for one node.
The node in the center represents the shaded region, so its weight is
proportional to the shaded area. The region is constructed by connecting the
...
{\it Examples with data, maybe \texttt{bei} dataset or Victorville. Show read
like a tutorial because there aren't many good resource yet.}
